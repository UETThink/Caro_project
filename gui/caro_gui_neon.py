# -*- coding: utf-8 -*-
"""
gui/caro_gui_neon.py
Giao diện Caro 9x9 với hiệu ứng Neon
- Menu chọn level trước khi chơi
"""
...
class NeonCaroGUI:
    ...
    def show_menu(self):
        """Hiển thị màn hình menu chọn level"""
        ...
        algo_title.pack(pady=15)

        algo_frame = tk.Frame(menu_frame, bg=self.bg_color)
        algo_frame.pack(pady=5)

        self.algo_var = tk.StringVar(value='alphabeta')

        minimax_btn = tk.Radiobutton(
            algo_frame,
            text="MINIMAX",
        ...
        minimax_btn.pack(side=tk.LEFT, padx=10)

        alphabeta_btn = tk.Radiobutton(
            algo_frame,
            text="ALPHA-BETA",
            variable=self.algo_var,
            value='alphabeta',
        ...
        alphabeta_btn.pack(side=tk.LEFT, padx=10)
    ...
    def start_game(self):
        """Bắt đầu game với cấp độ đã chọn"""
